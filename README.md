...
| **身体数据 (体脂秤)** | `body_measurements`| 手动称重时 | 除了基础的 `weight`, `bmi` 外，最新逆向已支持**全量高阶体脂数据**：`fatRate` (体脂率), `bodyWaterRate` (水分), `muscleRate` (肌肉量), `visceralFat` (内脏脂肪), `boneMass` (骨量 kg), `metabolism` (基础代谢 kcal), `proteinRatio` (蛋白质率), `skeletalMuscle` (骨骼肌 kg), `bodyBalanceScore` (身体平衡得分) 等 10+ 项指标！ |
